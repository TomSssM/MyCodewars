export default class GridSettBtn {
  constructor(data) {
  ...
  onSubmit(e) {
    ...
    GridSettBtn.validateInput(input);
    this.paint.createCanvas(input);
    this.hideEverything();
  }

  static validateInput(val) {
    if (Number.isNaN(val)) throw new Error('please enter a valid number');
    if (val < 1 || val > 32) throw new Error('range error');
    return true;
  }
