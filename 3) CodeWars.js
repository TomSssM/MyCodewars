...
function partitionOn(pred, items) {
  const trueValArr = items.filter(pred);
  const falseValArr = items.filter(v => !pred(v));
  const partitioned = [...falseValArr, ...trueValArr];
  items.forEach((v,i) => {
    items[i] = partitioned[i];
  });
  return falseValArr.length;
}
