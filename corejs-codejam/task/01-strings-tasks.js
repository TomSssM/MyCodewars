...
function encodeToRot13(str) {
    ...
    let posInAlph;
    let newPos;
    const keys = str.split('').map(char => char.charCodeAt(0));
    keys.forEach(key => {
        if(key <= 90 && key >= 65) {
            posInAlph = key - 65;
            newPos = (26 + posInAlph - 13) % 26;
            res += String.fromCharCode(newPos + 65);
        } else if (key <= 122 && key >= 97) {
            posInAlph = key - 97;
            newPos = (26 + posInAlph - 13) % 26;
            res += String.fromCharCode(newPos + 97);
        } else {
